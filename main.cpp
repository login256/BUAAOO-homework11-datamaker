...
const int PATHNODE = 80;

const int ADDTIMES = 35;
const int REMOVETIMES = 15;
const int LINETIMES = 1000;
const int TOTALTIMES = 6000;
...
public:
	...
	Path()
	{
		...
		{
			nodes.push_back(Node_Map::get_instance().map[dis(random_engine)]);
		}
	}
...
void connect_block_count()
{
	std::cout << "CONNECTED_BLOCK_COUNT" << std::endl;
}

void least_ticket_price()
{
	std::cout << "LEAST_TICKET_PRICE" << " " << gen_node_id(0.95) << " " << gen_node_id(0.95) << std::endl;
}

void least_transfer_count()
{
	std::cout << "LEAST_TRANSFER_COUNT" << " " << gen_node_id(0.95) << " " << gen_node_id(0.95) << std::endl;
}

void least_unpleasant_value()
{
	std::cout << "LEAST_UNPLEASANT_VALUE" << " " << gen_node_id(0.95) << " " << gen_node_id(0.95) << std::endl;
}
